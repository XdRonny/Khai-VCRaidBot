...
from Tech-Quard.Plguin.queue import QUEUE, add_to_queue, get_queue
...
async def ping(_, e: Message):
    ...
    if replied:
        if replied.audio or replied.voice:
            await e.delete()
            sumit = await replied.reply_text("**Transcoding Mp3...**")
        ...
        if len(e.command) < 2:
            ...
            await e.delete()
            sumit = await e.reply_text("__Searching...__")
            query = e.text.split(" ", 1)[1]
            search = ytsearch(query)
            if search == 0:
                await sumit.edit("`Didn't Find Anything for the Given Query`")
            else:
                songname = search[0]
                title = search[0]
                url = search[1]
                ...
                if hm == 0:
                    await sumit.edit(f"**YTDL ERROR ️!** \n\n`{ytlink}`")
                else:
                    if chat_id in QUEUE:
